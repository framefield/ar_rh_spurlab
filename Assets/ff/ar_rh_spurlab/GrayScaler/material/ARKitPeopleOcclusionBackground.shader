Shader "Unlit/ARKitPeopleOcclusionBackground"
{
    Properties
    {
        ...
        _stencilFactor ("StencilFactor", float) = 3.0
        _depthFactor ("DepthFactor", float) = 1.5
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            FragmentOutput frag (TexCoordInOut i)
            {
                ...
                float depth = (1.0 - _ZBufferParams.w * depthMeter) / (depthMeter * _ZBufferParams.z) / _depthFactor;

                // Get people segmentation
                half stencil = tex2D(_textureStencil, texcoord).r;
                stencil = step(1, stencil * _stencilFactor); // Get 0 or 1

                FragmentOutput o;

                float3 grayScale = dot(color.xyz, float3(0.3, 0.59, 0.11));
                o.color.rgb = lerp(grayScale, color.xyz, min(1, stencil) * _grayScaleStrength);
                o.color.a = stencil * 0.5f;
